...
int Galpani::primary_pseudo_source(Particle& particle, Particle& gcr_tmp_){
  ...
  if(2 == galdef.n_spatial_dimensions){
	...
	for(int ip = 0; ip < particle.n_pgrid; ++ip){
	  ...
	  particle.primary_source_function.d2[ir][iz].s[ip] -= 2*particle.Drz_wei.d2[ir][iz].s[ip]/4/particle.dr/particle.dz*(gcr_tmp_.cr_density.d2[ir+1][iz+1].s[ip]+gcr_tmp_.cr_density.d2[ir-1][iz-1].s[ip]-gcr_tmp_.cr_density.d2[ir-1][iz+1].s[ip]-gcr_tmp_.cr_density.d2[ir+1][iz-1].s[ip])*factor;
