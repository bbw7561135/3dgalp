...
int Galpani::create_gcr(){//AWS20050816
  //cout<<" >>>> create_gcr"<<endl;
  // modified by Wei Liu, 2018.12.17
#ifndef ITERAT
  INFO("Entry"); 
#else
...
//#pragma omp parallel for default(shared) schedule(dynamic) private(i,j,Z,A,Z1,A1,K_electron)
   for (int i = n_species-1; i >= 0; --i) {
      ...
      for (j = 0; j < n_species; ++j) {
         ...
         if (gcr[j].species != "nucleus" || gcr[j].A < 1)
            continue;

         cross_section = 0;

         if (gcr[j].A > gcr[i].A && !gcr[i].K_electron) {
            ...
            if (galdef.verbose==-602 ||galdef.verbose>=1) { // selectable debug                  //AWS20010828
               ...
               for (size_t ip = 0; ip < gcr[0].n_pgrid; buf << cross_section[ip++] << " "); 
               INFO(buf.str());
            ...
            for (size_t ip = 0; ip < gcr[i].n_pgrid; ++ip) {
         ...
         if (1 == galdef.K_capture) { //AWS20010828
            ...
            if (100*gcr[j].Z+gcr[j].A == 100*gcr[i].Z+gcr[i].A)
               if (gcr[j].K_electron != gcr[i].K_electron) {
                  ...
                  for (size_t ip = 0; ip < gcr[0].n_pgrid; ++ip) {
