...
extern int propel_arrays_initialize;

extern Distribution alpha1_r, alpha1_x, alpha1_y, alpha1_z, alpha1_p;
extern Distribution alpha2_r, alpha2_x, alpha2_y, alpha2_z, alpha2_p;
extern Distribution alpha3_r, alpha3_x, alpha3_y, alpha3_z, alpha3_p;
extern Distribution Nr1_, Nx1_, Ny1_, Nz1_, Np1_;
extern Distribution Nr2_, Nx2_, Ny2_, Nz2_, Np2_;
extern Distribution Nr3_, Nx3_, Ny3_, Nz3_, Np3_;
extern Distribution total_source_function;
extern Distribution Work;
...
extern double *particle_cr_density_arr;                                                        //AWS20110128
extern double *total_source_function_arr;                                                      //AWS20110128
extern double *alpha1_r_arr,*alpha1_x_arr,*alpha1_y_arr,*alpha1_z_arr,*alpha1_p_arr;           //AWS20110128
extern double *alpha2_r_arr,*alpha2_x_arr,*alpha2_y_arr,*alpha2_z_arr,*alpha2_p_arr;           //AWS20110128
extern double *alpha3_r_arr,*alpha3_x_arr,*alpha3_y_arr,*alpha3_z_arr,*alpha3_p_arr;           //AWS20110128
extern double *Work_arr;                                                                       //AWS20110128

//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|

int Galpani::propel(Particle& particle) {
    ...
    */
	...
	for (ip = 0; ip < particle.n_pgrid; ++ip) {
...
#ifdef ITERAT
	  alpha1_z.d2[ir][iz].s[ip] = particle.Dzz_wei.d2[ir][iz].s[ip]*pow(particle.dz,-2.);
	  alpha2_r.d2[ir][iz].s[ip] = particle.Drr_wei.d2[ir][iz].s[ip]*pow(particle.dr,-2.);
#else
	  ...
	  if (0 == ir) // use: Dxx/(R[i-1/2] dR)*{Ri(U[i+1]-Ui)-R[i-1](Ui-U[i-1])}; R[i-1]=0
	    // note by Wei Liu, 2018.12.22
	    // Dxx(∂(R∂ψ/∂R)/∂R)/R = Dxx/(RiΔR)(R[i+1/2](ψ[i+1]-ψ[i])/ΔR -R[i-1/2](ψ[i]-ψ[i-1])/ΔR)
	    // ∂ψ/∂R|(R=0) = 0 ⇒ ψ[i-1] = ψ[i+1]
	    // = Dxx/(RiΔR)(R[i+1/2](ψ[i+1]-ψ[i])/ΔR -R[i-1/2](ψ[i]-ψ[i-1])/ΔR)
	    // = Dxx/(RiΔR)(R[i+1/2]ψ[i+1]-(R[i+1/2]+R[i-1/2])ψ[i] +R[i-1/2]ψ[i-1])/ΔR
	    // = Dxx/(RiΔR²)(R[i+1/2]ψ[i+1]-(R[i+1/2]+R[i-1/2])ψ[i] +R[i-1/2]ψ[i-1])
	    // = Dxx/(RiΔR²)((R[i+1/2]++R[i-1/2])ψ[i+1]-(R[i+1/2]+R[i-1/2])ψ[i])
	    // = Dxx/(RiΔR²)(2R[i]ψ[i+1]-2R[i]ψ[i]) 
	    // = 2Dxx(ψ[i+1]-ψ[i])/ΔR²
...
#pragma omp parallel for schedule(runtime) default(shared) private(ir,iz,ip)
...
//                std::cout<<"ddxxdr="<<ddxxdr<<" ddxxdz="<<ddxxdz<<" alpha_r1="<<(alpha1_r.d2[ir][iz].s[ip])<<" alpha_z1="<<alpha1_z.d2[ir][iz].s[ip] <<std::endl;
                alpha1_r.d2[ir][iz].s[ip] -= dDxxdr*(1-particle.dr/particle.r[ir]);
                alpha3_r.d2[ir][iz].s[ip] += dDxxdr*(1+particle.dr/particle.r[ir]);
                alpha1_z.d2[ir][iz].s[ip] -= dDxxdz;
                alpha3_z.d2[ir][iz].s[ip] += dDxxdz;
#endif
            }//ip
        }//iz
    }//ir  
	...
	for (ip = 0; ip < particle.n_pgrid-1; ++ip) {
	  ...
	  for (ir = 0; ir < particle.n_rgrid; ++ir) {
        ...
        const double ap2 = particle.p[particle.n_pgrid-1]  /3.*galdef.dvdz_conv/(particle.p[particle.n_pgrid-1]-particle.p[particle.n_pgrid-2])*1.e5/kpc2cm;
	  ...
	  alpha2_p.d2[ir][iz].s[particle.n_pgrid-1] += ap2;
	...
	for (iz = 0; iz < particle.n_zgrid; ++iz) {
	  ...
	  for (ip = 0; ip < particle.n_pgrid-1; ++ip) {
	    ...
	    alpha3_p.d2[ir][iz].s[ip] += particle.dpdt.d2[ir][iz].s[ip+1]/(particle.p[ip+1] - particle.p[ip]);
          ...
          ip = particle.n_pgrid-1;
          alpha2_p.d2[ir][iz].s[ip] += particle.dpdt.d2[ir][iz].s[ip]/(particle.p[ip] - particle.p[ip-1]);
    ...
    */
    ...
    double f_use = 1./(galdef.prop_r + galdef.prop_z + galdef.prop_p); // used only by method = 1
     ...
     * This causes changes to the results because the source term is only added to a single diffusion dimension
    //AWS20110120
    ...
    */
    ...
    timestep_mode = 1;
    if ( dt < end_timestep_sec )
      timestep_mode = 2;
    
    for ( ; timestep_mode <= 2; ++timestep_mode) {
	...
	if (2 == timestep_mode)
	  ...
	  // following required to transfer from method=1 in timestep_mode 1
        ...
        Distribution m1alpha1_r, m1alpha1_z, m1alpha1_p;
        Distribution m1alpha2_r, m1alpha2_z, m1alpha2_p;
        Distribution m1alpha3_r, m1alpha3_z, m1alpha3_p;
        Distribution tsf_r, tsf_z, tsf_p;
        if (method == 1) {
           //Precalculate and re-order alphas for more efficient execution
           m1alpha1_r.init(particle.n_zgrid, particle.n_pgrid, particle.n_rgrid);
           m1alpha2_r.init(particle.n_zgrid, particle.n_pgrid, particle.n_rgrid);
           m1alpha3_r.init(particle.n_zgrid, particle.n_pgrid, particle.n_rgrid);
           m1alpha1_z.init(particle.n_rgrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha2_z.init(particle.n_rgrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha3_z.init(particle.n_rgrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha1_p.init(particle.n_rgrid, particle.n_zgrid, particle.n_pgrid);
           m1alpha2_p.init(particle.n_rgrid, particle.n_zgrid, particle.n_pgrid);
           m1alpha3_p.init(particle.n_rgrid, particle.n_zgrid, particle.n_pgrid);

           //Also pre-calculate and re-order the source function
           tsf_r.init(particle.n_zgrid, particle.n_pgrid, particle.n_rgrid);
           tsf_z.init(particle.n_rgrid, particle.n_pgrid, particle.n_zgrid);
           tsf_p.init(particle.n_rgrid, particle.n_zgrid, particle.n_pgrid);

           for (ir = 0; ir < particle.n_rgrid; ++ir) {
              for (iz = 0; iz < particle.n_zgrid; ++iz) {
                 for (ip = 0; ip < particle.n_pgrid; ++ip) {
                    m1alpha1_r.d2[iz][ip].s[ir] = 0.5*alpha1_r.d2[ir][iz].s[ip];
                    m1alpha2_r.d2[iz][ip].s[ir] = 1 - 0.5*alpha2_r.d2[ir][iz].s[ip];
                    m1alpha3_r.d2[iz][ip].s[ir] = 0.5*alpha3_r.d2[ir][iz].s[ip];
                    m1alpha1_z.d2[ir][ip].s[iz] = 0.5*alpha1_z.d2[ir][iz].s[ip];
                    m1alpha2_z.d2[ir][ip].s[iz] = 1 - 0.5*alpha2_z.d2[ir][iz].s[ip];
                    m1alpha3_z.d2[ir][ip].s[iz] = 0.5*alpha3_z.d2[ir][iz].s[ip];
                    m1alpha1_p.d2[ir][iz].s[ip] = 0.5*alpha1_p.d2[ir][iz].s[ip];
                    m1alpha2_p.d2[ir][iz].s[ip] = 1 - 0.5*alpha2_p.d2[ir][iz].s[ip];
                    m1alpha3_p.d2[ir][iz].s[ip] = 0.5*alpha3_p.d2[ir][iz].s[ip];

                    tsf_r.d2[iz][ip].s[ir] = total_source_function.d2[ir][iz].s[ip]*f_use;
                    tsf_z.d2[ir][ip].s[iz] = total_source_function.d2[ir][iz].s[ip]*f_use;
                    tsf_p.d2[ir][iz].s[ip] = total_source_function.d2[ir][iz].s[ip]*f_use;
                 }
              }
           }

           Nr1_ = m1alpha1_r;   Nr1_*= -1.0;
           Nr2_ = m1alpha2_r;   Nr2_*= -1.0;   Nr2_ += 2.0;   
           Nr3_ = m1alpha3_r;   Nr3_*= -1.0;
           Nz1_ = m1alpha1_z;   Nz1_*= -1.0;
           Nz2_ = m1alpha2_z;   Nz2_*= -1.0;   Nz2_ += 2.0;   
           Nz3_ = m1alpha3_z;   Nz3_*= -1.0;
           Np1_ = m1alpha1_p;   Np1_*= -1.0;
           Np2_ = m1alpha2_p;   Np2_*= -1.0;   Np2_ += 2.0;   
           Np3_ = m1alpha3_p;   Np3_*= -1.0;
        }
...
#pragma omp for schedule(runtime) 
	  for (ir = 0; ir < particle.n_rgrid; ++ir) {
		...
		Nz1[iz] = Nz1_.d2[ir][ip].s[iz];
		Nz2[iz] = Nz2_.d2[ir][ip].s[iz];
		Nz3[iz] = Nz3_.d2[ir][ip].s[iz];
		
		//		Nz0[iz] = total_source_function.d2[ir][iz].s[ip]*dt/f_use;
		Nz0[iz] = tsf_z.d2[ir][ip].s[iz];
		Nz0[iz] += m1alpha2_z.d2[ir][ip].s[iz]*particle.cr_density.d2[ir][iz].s[ip];
             
		if (iz > 0) 
		  Nz0[iz] += m1alpha1_z.d2[ir][ip].s[iz] 
		    *particle.cr_density.d2[ir][iz-1].s[ip];
                    
		if (iz < particle.n_zgrid-1) 
		  Nz0[iz] += m1alpha3_z.d2[ir][ip].s[iz] 
		    *particle.cr_density.d2[ir][iz+1].s[ip];
...
#pragma omp for schedule(runtime) 
	  for (ir = 0; ir < particle.n_rgrid; ++ir) {
		...
		Np0[ip] = tsf_p.d2[ir][iz].s[ip];
                        
		Np0[ip] += m1alpha2_p.d2[ir][iz].s[ip]
		  *particle.cr_density.d2[ir] [iz].s[ip];
                        
		if (ip > 0) 
		  Np0[ip] += m1alpha1_p.d2[ir][iz].s[ip] 
		    *particle.cr_density.d2[ir][iz].s[ip-1];
                    
		if (ip < particle.n_pgrid-1) 
		  Np0[ip] += m1alpha3_p.d2[ir][iz].s[ip] 
		    *particle.cr_density.d2[ir][iz].s[ip+1];
...
#pragma omp for schedule(runtime) 
		...
		Nr1[ir] = Nr1_.d2[iz][ip].s[ir];
		Nr2[ir] = Nr2_.d2[iz][ip].s[ir];
		Nr3[ir] = Nr3_.d2[iz][ip].s[ir];
		
		//		Nr0[ir] = total_source_function.d2[ir][iz].s[ip]*dt/f_use;
		Nr0[ir] = tsf_r.d2[iz][ip].s[ir];
	
		Nr0[ir] += m1alpha2_r.d2[iz][ip].s[ir]
		  *particle.cr_density.d2[ir][iz].s[ip];
                        
		if (ir > 0) 
		  Nr0[ir] += m1alpha1_r.d2[iz][ip].s[ir] 
		    *particle.cr_density.d2[ir-1][iz].s[ip];
                    
		if (ir < particle.n_rgrid-1) 
		  Nr0[ir] += m1alpha3_r.d2[iz][ip].s[ir] 
		    *particle.cr_density.d2[ir+1][iz].s[ip];
    ...
    */
	...
	for (iz = 0; iz < particle.n_zgrid; ++iz) {
	  ...
	  for (ip = 0; ip < particle.n_pgrid; ++ip) {
	    ...
	    // modified by Wei Liu, 2018.12.18
#ifdef ITERAT
	    alpha1_x.d3[ix][iy][iz].s[ip] = particle.Dxx_wei.d3[ix][iy][iz].s[ip]*pow(particle.dx, -2.);
	    alpha1_y.d3[ix][iy][iz].s[ip] = particle.Dyy_wei.d3[ix][iy][iz].s[ip]*pow(particle.dy, -2.);
	    alpha1_z.d3[ix][iy][iz].s[ip] = particle.Dzz_wei.d3[ix][iy][iz].s[ip]*pow(particle.dz, -2.);
#else
    ...
    // modification in propagation equation if diffusion coefficient depends on z,R
#pragma omp parallel for schedule(runtime) default(shared) private(ix,iy,iz,ip)
    for (ix = 1; ix < particle.n_xgrid-1; ++ix) {
       for (iy = 1; iy < particle.n_ygrid-1; ++iy) {
          for (iz = 1; iz < particle.n_zgrid-1; ++iz) {
	       ...
	       // modified by Wei Liu, 2018.12.18
#ifdef ITERAT
	       // ∂Dₓₓ/∂x·∂ψ/∂x = [Dₓₓ(i+1)-Dₓₓ(i-1)]·[ψ(i+1)-ψ(i-1)]/4/Δx/Δx
	      const double dDxxdx=(particle.Dxx_wei.d3[ix+1][iy][iz].s[ip]-particle.Dxx_wei.d3[ix-1][iy][iz].s[ip]) *pow(2.*particle.dx,-2.);
	      // ∂Dyy/∂y·∂ψ/∂y = [Dyy(j+1)-Dyy(j-1)]·[ψ(j+1)-ψ(j-1)]/4/Δy/Δy
	      const double dDyydy=(particle.Dyy_wei.d3[ix][iy+1][iz].s[ip]-particle.Dyy_wei.d3[ix][iy-1][iz].s[ip]) *pow(2.*particle.dy,-2.);
	      // ∂Dzz/∂z·∂ψ/∂z = [Dzz(k+1)-Dzz(k-1)]·[ψ(k+1)-ψ(k-1)]/4/Δz/Δz
	      const double dDzzdz=(particle.Dzz_wei.d3[ix][iy][iz+1].s[ip]-particle.Dzz_wei.d3[ix][iy][iz-1].s[ip]) *pow(2.*particle.dz,-2.); //

	      alpha1_x.d3[ix][iy][iz].s[ip] -= dDxxdx;
	      alpha3_x.d3[ix][iy][iz].s[ip] += dDxxdx;
	      alpha1_y.d3[ix][iy][iz].s[ip] -= dDyydy;
	      alpha3_y.d3[ix][iy][iz].s[ip] += dDyydy;
	      alpha1_z.d3[ix][iy][iz].s[ip] -= dDzzdz;
	      alpha3_z.d3[ix][iy][iz].s[ip] += dDzzdz;


	      // ∂Dₓy/∂x·∂ψ/∂y = [Dₓy(i+1)-Dₓy(i-1)]·[ψ(j+1)-ψ(j-1)]/4/Δx/Δy
	      const double dDxydx=(particle.Dxy_wei.d3[ix+1][iy][iz].s[ip]-particle.Dxy_wei.d3[ix-1][iy][iz].s[ip])/(4.*particle.dx*particle.dy); //
	      // ∂Dₓy/∂y·∂ψ/∂x = [Dₓy(j+1)-Dₓy(j-1)]·[ψ(i+1)-ψ(i-1)]/4/Δx/Δy
	      const double dDxydy=(particle.Dxy_wei.d3[ix][iy+1][iz].s[ip]-particle.Dxy_wei.d3[ix][iy-1][iz].s[ip])/(4.*particle.dx*particle.dy); // problem: outer	      

	      // ∂Dyz/∂y·∂ψ/∂z = [Dyz(j+1)-Dyz(j-1)]·[ψ(k+1)-ψ(k-1)]/4/Δy/Δz
	      const double dDyzdy=(particle.Dyz_wei.d3[ix][iy+1][iz].s[ip]-particle.Dyz_wei.d3[ix][iy-1][iz].s[ip])/(4.*particle.dy*particle.dz);
	      // ∂Dyz/∂z·∂ψ/∂y = [Dyz(k+1)-Dyz(k-1)]·[ψ(j+1)-ψ(j-1)]/4/Δy/Δz
	      const double dDyzdz=(particle.Dyz_wei.d3[ix][iy][iz+1].s[ip]-particle.Dyz_wei.d3[ix][iy][iz-1].s[ip])/(4.*particle.dy*particle.dz); //

	      // ∂Dzx/∂z·∂ψ/∂x = [Dzx(k+1)-Dzx(k-1)]·[ψ(i+1)-ψ(i-1)]/4/Δz/Δx
	      const double dDzxdz=(particle.Dzx_wei.d3[ix][iy][iz+1].s[ip]-particle.Dzx_wei.d3[ix][iy][iz-1].s[ip])/(4.*particle.dz*particle.dx); //
	      // ∂Dzx/∂x·∂ψ/∂z = [Dzx(i+1)-Dzx(i-1)]·[ψ(k+1)-ψ(k-1)]/4/Δz/Δx
	      const double dDzxdx=(particle.Dzx_wei.d3[ix+1][iy][iz].s[ip]-particle.Dzx_wei.d3[ix-1][iy][iz].s[ip])/(4.*particle.dz*particle.dx);

		// ∂Dₓy/∂y·∂ψ/∂x
                alpha1_x.d3[ix][iy][iz].s[ip] -= dDxydy;
                alpha3_x.d3[ix][iy][iz].s[ip] += dDxydy;
		// ∂Dₓy/∂x·∂ψ/∂y
                alpha1_y.d3[ix][iy][iz].s[ip] -= dDxydx;
                alpha3_y.d3[ix][iy][iz].s[ip] += dDxydx;

		// ∂Dyz/∂y·∂ψ/∂z
                alpha1_z.d3[ix][iy][iz].s[ip] -= dDyzdy;
                alpha3_z.d3[ix][iy][iz].s[ip] += dDyzdy;
		// ∂Dyz/∂z·∂ψ/∂y
                alpha1_y.d3[ix][iy][iz].s[ip] -= dDyzdz;
                alpha3_y.d3[ix][iy][iz].s[ip] += dDyzdz;

		// ∂Dzx/∂z·∂ψ/∂x
                alpha1_x.d3[ix][iy][iz].s[ip] -= dDzxdz;
                alpha3_x.d3[ix][iy][iz].s[ip] += dDzxdz;
		// ∂Dzx/∂x·∂ψ/∂z
                alpha1_z.d3[ix][iy][iz].s[ip] -= dDzxdx;
                alpha3_z.d3[ix][iy][iz].s[ip] += dDzxdx;
#else
                const double dDxxdx=(particle.Dxx.d3[ix+1][iy][iz].s[ip]-particle.Dxx.d3[ix-1][iy][iz].s[ip]) *pow(2.*particle.dx,-2.);
                const double dDxxdy=(particle.Dxx.d3[ix][iy+1][iz].s[ip]-particle.Dxx.d3[ix][iy-1][iz].s[ip]) *pow(2.*particle.dy,-2.);
                const double dDxxdz=(particle.Dxx.d3[ix][iy][iz+1].s[ip]-particle.Dxx.d3[ix][iy][iz-1].s[ip]) *pow(2.*particle.dz,-2.);

                alpha1_x.d3[ix][iy][iz].s[ip] -= dDxxdx;
                alpha3_x.d3[ix][iy][iz].s[ip] += dDxxdx;
                alpha1_y.d3[ix][iy][iz].s[ip] -= dDxxdy;
                alpha3_y.d3[ix][iy][iz].s[ip] += dDxxdy;
                alpha1_z.d3[ix][iy][iz].s[ip] -= dDxxdz;
                alpha3_z.d3[ix][iy][iz].s[ip] += dDxxdz;
#endif
             }//ip
          }//iz
       }//iy  
...
// DIFFUSIVE REACCELERATION   IMOS20020329
    ...
    double f_use = 1./(galdef.prop_x + galdef.prop_y + galdef.prop_z + galdef.prop_p);
     ...
     * Reverted as this causes changes in the solution
    //AWS20110120
    //method-independent alpha definitions without f_use, using alpha_2 only once AWS20110220
    ...
    */
    ...
    //Make it possible to skip timestep_mode = a all together
    timestep_mode = 1;
    if ( dt < end_timestep_sec )
      timestep_mode = 2;
	...
	if (2 == timestep_mode)
	  ...
	  // following required to transfer from method=1 in timestep_mode 1
        ...
        Distribution m1alpha1_x, m1alpha1_y, m1alpha1_z, m1alpha1_p;
        Distribution m1alpha2_x, m1alpha2_y, m1alpha2_z, m1alpha2_p;
        Distribution m1alpha3_x, m1alpha3_y, m1alpha3_z, m1alpha3_p;
        Distribution tsf_x, tsf_y, tsf_z, tsf_p;
        if (method == 1) {
           //Precalculate and re-order alphas for more efficient execution
           m1alpha1_x.init(particle.n_ygrid, particle.n_zgrid, particle.n_pgrid, particle.n_xgrid);
           m1alpha2_x.init(particle.n_ygrid, particle.n_zgrid, particle.n_pgrid, particle.n_xgrid);
           m1alpha3_x.init(particle.n_ygrid, particle.n_zgrid, particle.n_pgrid, particle.n_xgrid);
           m1alpha1_y.init(particle.n_xgrid, particle.n_zgrid, particle.n_pgrid, particle.n_ygrid);
           m1alpha2_y.init(particle.n_xgrid, particle.n_zgrid, particle.n_pgrid, particle.n_ygrid);
           m1alpha3_y.init(particle.n_xgrid, particle.n_zgrid, particle.n_pgrid, particle.n_ygrid);
           m1alpha1_z.init(particle.n_xgrid, particle.n_ygrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha2_z.init(particle.n_xgrid, particle.n_ygrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha3_z.init(particle.n_xgrid, particle.n_ygrid, particle.n_pgrid, particle.n_zgrid);
           m1alpha1_p.init(particle.n_xgrid, particle.n_ygrid, particle.n_zgrid, particle.n_pgrid);
           m1alpha2_p.init(particle.n_xgrid, particle.n_ygrid, particle.n_zgrid, particle.n_pgrid);
           m1alpha3_p.init(particle.n_xgrid, particle.n_ygrid, particle.n_zgrid, particle.n_pgrid);

           for (ix = 0; ix < particle.n_xgrid; ++ix) {
              for (iy = 0; iy < particle.n_ygrid; ++iy) {
                 for (iz = 0; iz < particle.n_zgrid; ++iz) {
                    for (ip = 0; ip < particle.n_pgrid; ++ip) {
                       m1alpha1_x.d3[iy][iz][ip].s[ix] = 0.5*alpha1_x.d3[ix][iy][iz].s[ip];
                       m1alpha2_x.d3[iy][iz][ip].s[ix] = 1 - 0.5*alpha2_x.d3[ix][iy][iz].s[ip];
                       m1alpha3_x.d3[iy][iz][ip].s[ix] = 0.5*alpha3_x.d3[ix][iy][iz].s[ip];

		       // modified by Wei Liu, 2018.12.18
#ifdef ITERAT
		       m1alpha1_y.d3[ix][iz][ip].s[iy] = 0.5*alpha1_y.d3[ix][iy][iz].s[ip];
                       m1alpha2_y.d3[ix][iz][ip].s[iy] = 1 - 0.5*alpha2_y.d3[ix][iy][iz].s[ip];
                       m1alpha3_y.d3[ix][iz][ip].s[iy] = 0.5*alpha3_y.d3[ix][iy][iz].s[ip];
#else
                       m1alpha1_y.d3[ix][iz][ip].s[iy] = 0.5*alpha1_x.d3[ix][iy][iz].s[ip];
                       m1alpha2_y.d3[ix][iz][ip].s[iy] = 1 - 0.5*alpha2_x.d3[ix][iy][iz].s[ip];
                       m1alpha3_y.d3[ix][iz][ip].s[iy] = 0.5*alpha3_x.d3[ix][iy][iz].s[ip];
#endif
                       m1alpha1_z.d3[ix][iy][ip].s[iz] = 0.5*alpha1_z.d3[ix][iy][iz].s[ip];
                       m1alpha2_z.d3[ix][iy][ip].s[iz] = 1 - 0.5*alpha2_z.d3[ix][iy][iz].s[ip];
                       m1alpha3_z.d3[ix][iy][ip].s[iz] = 0.5*alpha3_z.d3[ix][iy][iz].s[ip];
                       m1alpha1_p.d3[ix][iy][iz].s[ip] = 0.5*alpha1_p.d3[ix][iy][iz].s[ip];
                       m1alpha2_p.d3[ix][iy][iz].s[ip] = 1 - 0.5*alpha2_p.d3[ix][iy][iz].s[ip];
                       m1alpha3_p.d3[ix][iy][iz].s[ip] = 0.5*alpha3_p.d3[ix][iy][iz].s[ip];
           ...
           Nx1_ = m1alpha1_x;   Nx1_*= -1.0;
           Nx2_ = m1alpha2_x;   Nx2_*= -1.0;   Nx2_ += 2.0;   
           Nx3_ = m1alpha3_x;   Nx3_*= -1.0;
           Ny1_ = m1alpha1_y;   Ny1_*= -1.0;
           Ny2_ = m1alpha2_y;   Ny2_*= -1.0;   Ny2_ += 2.0;   
           Ny3_ = m1alpha3_y;   Ny3_*= -1.0;
           Nz1_ = m1alpha1_z;   Nz1_*= -1.0;
           Nz2_ = m1alpha2_z;   Nz2_*= -1.0;   Nz2_ += 2.0;   
           Nz3_ = m1alpha3_z;   Nz3_*= -1.0;
           Np1_ = m1alpha1_p;   Np1_*= -1.0;
...
#pragma omp for schedule(runtime)
		...
		for (iz = 0; iz < particle.n_zgrid; ++iz) {
	      ...
	      for (ip = 0; ip < particle.n_pgrid; ++ip) {
			...
			Nx1[ix] = Nx1_.d3[iy][iz][ip].s[ix];
			Nx2[ix] = Nx2_.d3[iy][iz][ip].s[ix];
			Nx3[ix] = Nx3_.d3[iy][iz][ip].s[ix];
			Nx0[ix] = total_source_function.d3[ix][iy][iz].s[ip]*f_use;

			Nx0[ix] += m1alpha2_x.d3[iy][iz][ip].s[ix]*particle.cr_density.d3[ix][iy][iz].s[ip];
                     
			if (ix > 0) 
			  Nx0[ix] += m1alpha1_x.d3[iy][iz][ip].s[ix]*particle.cr_density.d3[ix-1][iy][iz].s[ip];
                    
			if (1 == galdef.use_symmetry && 0 == ix) 
			  Nx0[ix] += m1alpha1_x.d3[iy][iz][ip].s[ix]*particle.cr_density.d3[ix+1][iy][iz].s[ip];
                                 
			if (ix < particle.n_xgrid-1)
			  Nx0[ix] += m1alpha3_x.d3[iy][iz][ip].s[ix]*particle.cr_density.d3[ix+1][iy][iz].s[ip];
...
#pragma omp for schedule(runtime)
		...
		for (iz = 0; iz < particle.n_zgrid; ++iz) {
	      ...
	      for (ip = 0; ip < particle.n_pgrid; ++ip) {
			...
			Ny1[iy] = Ny1_.d3[ix][iz][ip].s[iy];
			Ny2[iy] = Ny2_.d3[ix][iz][ip].s[iy];
			Ny3[iy] = Ny3_.d3[ix][iz][ip].s[iy];
			Ny0[iy] = total_source_function.d3[ix][iy][iz].s[ip]*f_use;
                        
			Ny0[iy] += m1alpha2_y.d3[ix][iz][ip].s[iy]*particle.cr_density.d3[ix][iy][iz].s[ip];
                         
			if (iy > 0) 
			  Ny0[iy] += m1alpha1_y.d3[ix][iz][ip].s[iy]*particle.cr_density.d3[ix][iy-1][iz].s[ip];

			if (1 == galdef.use_symmetry && 0 == iy) 
			  Ny0[iy] += m1alpha1_y.d3[ix][iz][ip].s[iy]*particle.cr_density.d3[ix][iy+1][iz].s[ip];

			if (iy < particle.n_ygrid-1) 
			  Ny0[iy] += m1alpha3_y.d3[ix][iz][ip].s[iy]*particle.cr_density.d3[ix][iy+1][iz].s[ip];
...
#pragma omp for schedule(runtime)
		for (ix = 0; ix < particle.n_xgrid; ++ix) {
	      ...
	      for (ip = 0; ip < particle.n_pgrid; ++ip) {
			...
			Nz1[iz] = Nz1_.d3[ix][iy][ip].s[iz];
			Nz2[iz] = Nz2_.d3[ix][iy][ip].s[iz];
			Nz3[iz] = Nz3_.d3[ix][iy][ip].s[iz];
			Nz0[iz] = total_source_function.d3[ix][iy][iz].s[ip]*f_use;

			Nz0[iz] += m1alpha2_z.d3[ix][iy][ip].s[iz]*particle.cr_density.d3[ix][iy][iz].s[ip];
                            
			if (iz > 0)
			  Nz0[iz] += m1alpha1_z.d3[ix][iy][ip].s[iz] *particle.cr_density.d3[ix][iy][iz-1].s[ip];
			
			if (galdef.use_symmetry && iz==0)
			  Nz0[iz] += m1alpha1_z.d3[ix][iy][ip].s[iz]*particle.cr_density.d3[ix][iy][iz+1].s[ip];

			if (iz <particle.n_zgrid-1)
			  Nz0[iz] += m1alpha3_z.d3[ix][iy][ip].s[iz] *particle.cr_density.d3[ix][iy][iz+1].s[ip];
...
#pragma omp for schedule(runtime)
		...
		for (iy = 0; iy < particle.n_ygrid; ++iy) {
		  ...
		  for (iz = 0; iz < particle.n_zgrid; ++iz) {
			...
			Np0[ip] = total_source_function.d3[ix][iy][iz].s[ip]*f_use;
                                 
			Np0[ip] += m1alpha2_p.d3[ix][iy][iz].s[ip]*particle.cr_density.d3[ix][iy][iz].s[ip];
			
			if (ip > 0)
			  Np0[ip] += m1alpha1_p.d3[ix][iy][iz].s[ip]*particle.cr_density.d3[ix][iy][iz].s[ip-1];
			
			if (ip < particle.n_pgrid-1)
			  Np0[ip] += m1alpha3_p.d3[ix][iy][iz].s[ip]*particle.cr_density.d3[ix][iy][iz].s[ip+1];
