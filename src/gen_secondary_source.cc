...
*   filename : gen_secondary_source.cc
*   description :
*   
*   current version : 1.0
*   author : Wei Liu
...
int Galpani::gen_secondary_source(Particle& particle){
  ...
  */
  ...
  for(int i=n_species-1; i>=0; --i)
    if(particle.name == gcr_tmp[i].name)
      i_species = i;

  secondary_pseudo_source(particle, gcr_tmp[i_species]);
#endif
